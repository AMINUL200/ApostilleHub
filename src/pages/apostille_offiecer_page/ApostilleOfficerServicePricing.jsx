...
const ApostilleOfficerServicePricing = () => {
  ...
  // Fetch profile and pricings on mount
  useEffect(() => {
    fetchProfile();
    fetchPricings();
  }, []);
  ...
  const fetchPricings = async () => {
    try {
      const response = await api.get('/apostille-officer/service-pricings');
      if (response.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingPricing) {
        response = await api.put(`/apostille-officer/service-pricings/${editingPricing.id}`, payload);
      } else {
        response = await api.post('/apostille-officer/service-pricings', payload);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await api.delete(`/apostille-officer/service-pricings/${selectedPricing.id}`);
  ...
  const handleStatusToggle = async (pricing) => {
    ...
    try {
      ...
      const response = await api.put(`/apostille-officer/service-pricings/${pricing.id}`, payload);
  ...
  const handleBack = () => {
    navigate('/apostille-officer/dashboard');
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: '#64748B' }}>
                  {profile?.professional_name || 'Loading...'} - Manage your service pricing
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              >
                {profile.profile_photo ? (
                  ...
                  getInitials(profile.professional_name || profile.user?.name)
                )}
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-semibold" style={{ color: '#0B1220' }}>
                  {profile.professional_name || profile.user?.name}
                </h3>
                <div className="flex flex-wrap items-center gap-4 mt-1">
                  ...
                  <span className="text-sm" style={{ color: '#64748B' }}>
                    ...
                    {profile.law_firm_name || 'Independent'}
                  </span>
                  <span className="text-sm" style={{ color: '#64748B' }}>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium px-3 py-1 rounded-full" style={{ 
                  background: profile.approval_status === 'approved' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                  color: profile.approval_status === 'approved' ? '#10B981' : '#EF4444'
                }}>
                  {profile.approval_status?.toUpperCase() || 'PENDING'}
                </span>
                <span className="text-xs font-medium px-3 py-1 rounded-full" style={{ 
                  background: profile.is_available ? 'rgba(16, 185, 129, 0.1)' : 'rgba(148, 163, 184, 0.1)',
                  color: profile.is_available ? '#10B981' : '#94A3B8'
                }}>
...
export default ApostilleOfficerServicePricing;